...
app.get("/JokeFilter",(req,res)=>{
  const type = req.query.type;
  const joke = jokes.filter((joke)=>joke.jokeType===type);
  res.json(joke);
})
...
//5. PUT a joke
app.put("/jokesPut/:id",(req,res)=>{
  const id = parseInt(req.params.id);
  
  const jokeUpdated = jokes.find((joke)=> joke.id===id);
  jokeUpdated.jokeText= req.body.text;
  jokeUpdated.jokeType= req.body.type;
  res.json(jokeUpdated);
})

//6. PATCH a joke
app.patch("/jokesPatch/:id",(req,res)=>{
  const id = parseInt(req.params.id);
  const exsistingText = jokes.find((joke)=>joke.id===id);
  const replacedText={
    id:id,
    jokeText: req.body.text || exsistingText.jokeText,
    jokeType: req.body.type|| exsistingText.jokeType,
  }
  const index = jokes.findIndex((joke)=>joke.id===id);
  jokes[index]=replacedText;
  res.json(jokes);
})

//7. DELETE Specific joke
app.delete("/jokesDelete/:id",(req,res)=>{
  const id =parseInt( req.params.id);
